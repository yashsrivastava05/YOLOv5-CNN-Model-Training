...
def train_yolov5(data_yaml, weights="yolov5m.pt", img_size=416, batch_size=16, epochs=50, device=0, project_path="/drive/MyDrive/new0yolo/weights/new_weights", hyp_path="yolov5/data/hyps/hyp.scratch-low.yaml"):
    """
    Trains YOLOv5 using specified parameters for a multi-class dataset with augmentations.
    """
    command = [
...
# Train YOLOv5 for multiple classes with augmentation
data_yaml = "/drive/MyDrive/new0yolo/newDATASET/dataset.yaml"  # Ensure the dataset.yaml is configured correctly
hyp_path = "/drive/MyDrive/new0yolo/cust_hyperameter/cust_hyps.yaml"  # Path to your custom hyp.yaml file
train_yolov5(data_yaml, hyp_path=hyp_path)
